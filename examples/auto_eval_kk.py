...
# ==================== only edit these 3 ====================
MODEL_NAME_PREFIX = "1.7b-oracle-mask-kk"
CHECKPOINT_DIR = "/mnt/dolphinfs/ssd_pool/docker/user/hadoop-xt-ai-search/ai-search/wangli/save_checkpoints/checkpoints/grpo_adv_diff_top_gt-top_0.2-softadv_100.0-1.7b-kk"
HF_MODEL_PATH = "/mnt/dolphinfs/ssd_pool/docker/user/hadoop-xt-ai-search/ai-search/wangli/llm_model/Qwen3-1.7B-Base"
# ==================== defaults (aligned with eval.sh) ====================
REPO_ROOT = Path(__file__).resolve().parents[1]
REWARD_FUNCTION_PATH = REPO_ROOT / "verl" / "utils" / "reward_score" / "kk.py"
...
DATASET_NAMES = ["3ppl_test", "4ppl_test", "5ppl_test", "6ppl_test", "7ppl_test", "8ppl_test"]
STEPS = None
...
def _eval_cmd(actor_model_path: Path, exp_name: str):
    train_files = _hydra_list([str(DATA_DIR / "train_data" / "kk_train.parquet")])
    ...
    cmd = [
        ...
        "+trainer.val_dataset_rollout_n.kk_logic_3ppl=8",
        "+trainer.val_dataset_rollout_n.kk_logic_4ppl=8",
        "+trainer.val_dataset_rollout_n.kk_logic_5ppl=8",
        "+trainer.val_dataset_rollout_n.kk_logic_6ppl=8",
        "+trainer.val_dataset_rollout_n.kk_logic_7ppl=8",
        "+trainer.val_dataset_rollout_n.kk_logic_8ppl=8",
...
def _scores(tb_dir: Path):
    ...
    dataset_acc = {
        ...
        if tag.startswith("val-core/kk_logic_") and "/acc/mean@" in tag
    }
    if dataset_acc:
        res["average_accuracy"] = sum(dataset_acc.values()) / len(dataset_acc)
    elif res:
