...
# Fraction p of prompts with the lowest average response probability that use true GT label each step.
# When AVG_PROB_GT_MASK_NON_GT=False, non-GT prompts train with majority-vote GT instead of being masked.
export AVG_PROB_GT_TOP_PCT=${AVG_PROB_GT_TOP_PCT:-0.2}
export AVG_PROB_GT_MASK_NON_GT=${AVG_PROB_GT_MASK_NON_GT:-True}
export REWARD_TYPE=avg_prob_gt_zero_adv
PCT_TAG=${AVG_PROB_GT_TOP_PCT/./p}
MASK_NON_GT_TAG=${AVG_PROB_GT_MASK_NON_GT}

export TENSORBOARD_DIR="xxx/al-rlvr/tensorboard_log/train_log/8b_${REWARD_TYPE}_top_${PCT_TAG}_mask_non_gt_${MASK_NON_GT_TAG}"
...
export EXPERIMENT_NAME=${REWARD_TYPE}-top_${PCT_TAG}-mask_non_gt_${MASK_NON_GT_TAG}-8b
if [[ "$EXPERIMENT_NAME" != *kk* && "$EXPERIMENT_NAME" != *KK* ]]; then
    export EXPERIMENT_NAME="${EXPERIMENT_NAME}-kk"
fi

ROLLOUT_DIR=xxx/al-rlvr/logs/8b_${REWARD_TYPE}_top_${PCT_TAG}_mask_non_gt_${MASK_NON_GT_TAG}
...
echo "[avg-prob-gt-zero-adv] AVG_PROB_GT_TOP_PCT=${AVG_PROB_GT_TOP_PCT}"
echo "[avg-prob-gt-zero-adv] AVG_PROB_GT_MASK_NON_GT=${AVG_PROB_GT_MASK_NON_GT}"
...
python3 -m verl.trainer.main_ppo \
    ...
    unsupervised_reward.avg_prob_gt_top_pct=$AVG_PROB_GT_TOP_PCT \
    unsupervised_reward.avg_prob_gt_mask_non_gt=$AVG_PROB_GT_MASK_NON_GT \
