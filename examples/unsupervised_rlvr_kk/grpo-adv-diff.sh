...
export TENSORBOARD_DIR="xxx/al-rlvr/tensorboard_log/train_log/8b_oracle_adv${ORACLE_GT_TOP_PCT}_softadv_${SOFT_ADV}"
...
export EXPERIMENT_NAME=${REWARD_TYPE}-top_${GRPO_ADV_TOP_PCT}-softadv_${SOFT_ADV}-8b
if [[ "$EXPERIMENT_NAME" != *kk* && "$EXPERIMENT_NAME" != *KK* ]]; then
    export EXPERIMENT_NAME="${EXPERIMENT_NAME}-kk"
fi

ROLLOUT_DIR=xxx/al-rlvr/logs/phi_oracle_adv${ORACLE_GT_TOP_PCT}_softadv_${SOFT_ADV}
