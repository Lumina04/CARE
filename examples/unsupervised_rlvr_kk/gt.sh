...
export DATA_DIR=xxx
export TENSORBOARD_DIR="xxx/al-rlvr/tensorboard_log/train_log/8b-kk-gt"
if [[ "$TENSORBOARD_DIR" != *kk* && "$TENSORBOARD_DIR" != *KK* ]]; then
    export TENSORBOARD_DIR="${TENSORBOARD_DIR}-kk"
fi
export REWARD_TYPE=kk_gt

ZERO=False
export MAX_RESP_LENGTH=${MAX_RESP_LENGTH:-4000}
export MAX_VAL_RESP_LENGTH=${MAX_VAL_RESP_LENGTH:-4000}
export MINI_BATCH_SIZE=${MINI_BATCH_SIZE:-32}
export TEMPERATURE=${TEMPERATURE:-1.0}
export N_RESPONSES=${N_RESPONSES:-8}
export USE_KL=${USE_KL:-True}

export EXPERIMENT_NAME=${EXPERIMENT_NAME:-${REWARD_TYPE}-8b-kk}
if [[ "$EXPERIMENT_NAME" != *kk* && "$EXPERIMENT_NAME" != *KK* ]]; then
    export EXPERIMENT_NAME="${EXPERIMENT_NAME}-kk"
fi

# ROLLOUT_DIR=xxx/al-rlvr/logs/gt-8b-kk
...
export ACTOR_MODEL_PATH=${ACTOR_MODEL_PATH:-xxx/llm_model/Qwen3-8B-Base}
export PROJECT_PATH=${PROJECT_PATH:-xxx/save_checkpoints}
...
echo "KK reward: $REPO_ROOT/verl/utils/reward_score/kk.py"
