...
cd "$REPO_ROOT"
export DATA_DIR=xxx
export TENSORBOARD_DIR="${TENSORBOARD_DIR:-xxx/al-rlvr/tensorboard_log/train_log/8b_ttrl}"
...
export ZERO_WRONG_MAJORITY_ADV=${ZERO_WRONG_MAJORITY_ADV:-True}

export EXPERIMENT_NAME="${EXPERIMENT_NAME:-${REWARD_TYPE}-8b}"
if [[ "$EXPERIMENT_NAME" != *kk* && "$EXPERIMENT_NAME" != *KK* ]]; then
    export EXPERIMENT_NAME="${EXPERIMENT_NAME}-kk"
fi

# ROLLOUT_DIR=xxx/al-rlvr/logs/8b_ttrl
...
python3 -m verl.trainer.main_ppo \
    ...
    unsupervised_reward.zero_wrong_majority_adv=$ZERO_WRONG_MAJORITY_ADV \
