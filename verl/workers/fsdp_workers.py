...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def get_prompt_hidden_size(self) -> int:
        assert self._is_actor, "get_prompt_hidden_size requires actor role"

        hidden_size = None
        model_cfg = getattr(self, "actor_model_config", None)
        if model_cfg is not None:
            hidden_size = getattr(model_cfg, "hidden_size", None)
            if hidden_size is None:
                text_cfg = getattr(model_cfg, "text_config", None)
                hidden_size = getattr(text_cfg, "hidden_size", None) if text_cfg is not None else None

        if hidden_size is None:
            actor_module_cfg = getattr(getattr(self, "actor_module", None), "config", None)
            if actor_module_cfg is not None:
                hidden_size = getattr(actor_module_cfg, "hidden_size", None)
                if hidden_size is None:
                    text_cfg = getattr(actor_module_cfg, "text_config", None)
                    hidden_size = getattr(text_cfg, "hidden_size", None) if text_cfg is not None else None

        if hidden_size is None:
            raise RuntimeError("Unable to infer actor hidden_size from actor model config")
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        # Entropy is kept on by default for actor/entropy logging; callers that only need log-probs can skip it.
        ...
        with self.ulysses_sharding_manager:
            ...
            with adapter_ctx:
                output, entropys, self_certaintys = self.actor.compute_log_prob(
                    data=data, calculate_entropy=calculate_entropy, calculate_self_certainty=calculate_self_certainty
                )
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    @DistProfiler.annotate(color="blue")
    def compute_probe_stage1_features(self, data: DataProto):
        assert self._is_actor
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp)
        ...
        data.meta_info["micro_batch_size"] = int(micro_batch_size)
        data.meta_info["temperature"] = self.config.rollout.temperature
        data.meta_info["use_dynamic_bsz"] = self.config.rollout.log_prob_use_dynamic_bsz
        data.meta_info["max_token_len"] = self.config.rollout.log_prob_max_token_len_per_gpu
        ...
        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data)
            probe_feature_tensors = self.actor.compute_probe_stage1_features(data=data)
            output = DataProto.from_dict(tensors=probe_feature_tensors)
            output = self.ulysses_sharding_manager.postprocess_data(output)
        ...
        if self._is_offload_param:
            offload_fsdp_model_to_cpu(self.actor_module_fsdp)
            log_gpu_memory_usage("After offload actor model during compute_probe_stage1_features", logger=logger)
